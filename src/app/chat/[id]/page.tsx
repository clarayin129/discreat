...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    setTimeout(async () => {
      const reply = `Help dispatched to ${report?.address || "unknown location"} for report ${id}`
      await fetch("/api/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex gap-4 mb-6">
        <button onClick={() => router.push("/dashboard")} className="bg-gray-200 px-4 py-2 rounded text-sm">
          ← Back to Dashboard
        </button>
        <button onClick={() => router.push(`/dashboard/${id}`)} className="bg-blue-100 px-4 py-2 rounded text-sm">
          📄 View Report Details
        </button>
      </div>
